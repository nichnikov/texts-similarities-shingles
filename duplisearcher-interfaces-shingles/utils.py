...
def remote_duplisearcher(args, sending_url, upload_type="excel"):
    ...
    if upload_type == "json":
        ...
        json_data = {"Texts1": list(df1[0]), "Texts2": list(df2[0]), "Score": args['score'],
...
def response_func(clustering_texts_df, response_type="excel"):
    ...
    if response_type == "excel":
        headers.add('Content-Disposition', 'attachment', filename="searching_results.xlsx")
        buffer = io.BytesIO()
        ...
        headers.add('Content-Disposition', 'attachment', filename="searching_results.csv")
